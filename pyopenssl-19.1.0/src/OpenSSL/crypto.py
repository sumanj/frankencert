...
class X509Extension(object):
    ...
    def get_oid(self):
        ...
        :return: A Cstring giving the X509Extension's ASN.1 encoded data.
        :rtype: :py:class:`str` type

        .. versionadded:: Custom for Frankencert
        """
        obj = _lib.X509_EXTENSION_get_object(self._extension)
        result_buffer = _ffi.new("unsigned char[]", 512)
        result_length = _lib.OBJ_obj2txt(result_buffer, len(result_buffer), obj, 1)
        return _native(_ffi.string(result_buffer))

    def set_critical(self, critical):
        """
        Set the critical field of this X.509 extension

        .. versionadded:: Custom for Frankencert
        """
        obj = _lib.X509_EXTENSION_get_object(self._extension)
        data = _lib.X509_EXTENSION_get_data(self._extension)
        # lib does not contain X509_EXTENSION_set_context. Rebuild the extension
