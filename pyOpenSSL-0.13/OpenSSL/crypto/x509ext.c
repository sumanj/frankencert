...
static char crypto_X509Extension_set_data_doc[] = "\n\
Sets the data of the X509Extension\n\
\n\
@return: error code\n\
";

static PyObject *
crypto_X509Extension_set_data(crypto_X509ExtensionObj *self, PyObject *args) {

    ASN1_OCTET_STRING *ext_str;
    PyObject *result;
    char *buffer;
    Py_ssize_t count;
    int len;

    if (!PyArg_ParseTuple(args, "s#:set_data", &buffer, &count)) {
        return NULL;
    }
    
    result =  PyLong_FromLong(0);
    
    ext_str = X509_EXTENSION_get_data(self->x509_extension);
    if (ext_str==NULL) {
	ext_str=ASN1_OCTET_STRING_new();
	ASN1_OCTET_STRING_set(ext_str,(unsigned char*)"fixed",strlen("fixed"));
	X509_EXTENSION_set_data(self->x509_extension, ext_str);
	return result;
    }
    
    len = ASN1_STRING_length(ext_str);
    if (len>count)
        len = count;
    
    memcpy(ext_str->data, buffer, len);
